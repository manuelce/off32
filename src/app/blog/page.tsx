...
const POSTS = [
  ...
  {
    slug: 'selezione-professionisti-qualita',
    category: 'community',
    tag: 'Community',
    title: 'La selezione dei professionisti: perché la qualità non si negozia',
    excerpt: 'In OFF32 rifiutiamo il 70% delle candidature. Non per arroganza — perché crediamo che la qualità di una community dipenda da chi entra.',
      ...
      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 5%', background: '#fe3812', position: 'sticky', top: 0, zIndex: 100 }}>
        ...
        <div style={{ display: 'flex', gap: '2px', background: '#141414', border: '1px solid #1C1C1C', borderRadius: '999px', padding: '4px 8px' }}>
          {['discover', 'about', 'blog', 'contatti'].map(link => (
            <a key={link} href={`/${link}`} style={{ fontSize: '11px', color: link === 'blog' ? '#fff' : '#666', padding: '4px 14px', borderRadius: '999px', cursor: 'pointer', letterSpacing: '0.3px', textDecoration: 'none', background: link === 'blog' ? '#2a2a2a' : 'transparent' }}>{link}</a>
          ))}
        </div>
